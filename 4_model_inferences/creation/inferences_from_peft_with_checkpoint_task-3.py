...
def generate_over_a_t1_dataset(model, tokenizer, dataset, base_model, task, sample_type_name, nb_sample, loss_computation, config):
    new_dataset = []
    ...
    for index,sample in enumerate(dataset):
        print(index)
        #informations are first gathered (model, task, sample_type, nb samples, loss, config) this will be useful to keep track of the fine-tuned model used
        new_sample = {"base_model":base_model,"task":task, "sample_type":sample_type_name, "nb_samples":nb_sample, 
                      "loss_computation":loss_computation, "config":config}
...
def generate_over_a_t2_dataset(model, tokenizer, dataset, base_model, task, sample_type_name, nb_sample, loss_computation, config):
    new_dataset = []
...
def generate_over_a_t3_dataset(model, tokenizer, dataset, base_model, task, sample_type_name, nb_sample, loss_computation, config):
    new_dataset = []
...
def generate_over_dataset(model, tokenizer, dataset, base_model, task, sample_type_name, nb_sample, loss_computation, config):
    if task=="task-1":
        results = generate_over_a_t1_dataset(model=model, tokenizer=tokenizer, dataset=dataset, 
                                                            ...
                                                            nb_sample=nb_sample, loss_computation=loss_computation, config=config)
    elif task=="task-2":
...
if __name__=="__main__":
    ...
    #list used to skip models where inferences were already made
    skip_list=["GaetanMichelet/Mistral-7B_task-3_60-samples_config-1",
               "GaetanMichelet/Mistral-7B_task-3_60-samples_config-2",
               "GaetanMichelet/Mistral-7B_task-3_60-samples_config-1_full",
               "GaetanMichelet/Mistral-7B_task-3_60-samples_config-2_full"]

    #base_models = ["meta-llama/Meta-Llama-3.1-8B-Instruct","google/gemma-2-2b-it","mistralai/Mistral-7B-Instruct-v0.3"]
    base_models = ["mistralai/Mistral-7B-Instruct-v0.3"]
    tasks = ["task-3"]
    ...
    for base_model in base_models:
        ...
        for task in tasks:
            ...
            for sample_type in sample_types:
                ...
                testing_dataset_path = f"./rerun_task-3/{task}/{sample_type_name}/test.json"

                #creates the output file (add the model name)
                testing_dataset_with_inferences_path = f"./output_rerun_task-3/{base_model_name}/{task}/{sample_type_name}/test_with_inferences.json"
                ...
                #loop over the nb samples, loss computation, config elements that vary according to the fine-tuned model
                for nb_sample in nb_samples:
                    for loss_computation in loss_computations:
                        for config in configs:

                            #retrieves the correct fine-tuned model
                            adapter_name = f"GaetanMichelet/{base_model_name}_{task}_{nb_sample}_{config}{loss_computation}{sample_type}"

    #######################################################################

                            print(f"{datetime.now().strftime('%d.%m.%Y %H:%M:%S')} - Launching inference for adapters {adapter_name}")

                            #load the base model and the adapters to form the fine-tuned model
                            base_model_model = AutoModelForCausalLM.from_pretrained(base_model, torch_dtype=torch.bfloat16)
                            model = PeftModel.from_pretrained(base_model_model, adapter_name)
                            model = model.to("cuda")
                            ...
                            #if the current model is not present in the list to skip
                            if adapter_name not in skip_list:
