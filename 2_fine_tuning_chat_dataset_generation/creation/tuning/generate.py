...
def generate_message_summary_prompt_v2_task_1(type_sample,json_dataset_sample,
    ...
    prompt='\n'.join([introduction_messages_text, json_dataset_sample["own"]["new_dialogue"], request_text])
    if type_sample=="manual":
        answer = json_dataset_sample["own"]["new_summaries"]["new_precise"]
    elif type_sample=="automatic":
        answer = json_dataset_sample["own"]["new_summaries"]["new_GPT4_adjusted"]
    else:
...
def generate_message_summary_prompt_v2_task_2(type_sample,json_dataset_sample,
    ...
    prompt_1 = '\n'.join([initial_introduction_messages_text, json_dataset_sample["combined"]["new_dialogue"].replace("\n\n","\r\n"), initial_request_text])
    prompt_2 = '\n'.join([second_introduction_messages_text, json_dataset_sample["combined"]["prompt_2"], second_request_text])
    if type_sample=="manual":
        answer_1 = json_dataset_sample["combined"]["new_summary"]
        answer_2 = json_dataset_sample["own"]["new_summaries"]["new_precise"]
    elif type_sample == "automatic":
        answer_1 = json_dataset_sample["combined"]["new_GPT4_summary"]
        answer_2 = json_dataset_sample["own"]["new_summaries"]["new_GPT4_adjusted"]
    else:
        print("Problem with the type")
...
def generate_message_summary_prompt_v2_task_3(type_sample,json_dataset_sample,
    ...
    list_of_all_crimes = ["drug trafficking or dealing activities","heist activities","burglary activities", 
                      "burglary activities currently taking place", "drug dealing activities", "heist that are currently taking place", 
                      "heist currently in progress", "scam activities", "drug manufacturing operation"]
    
    list_of_crimes = ["drug", "heist", "burglary", "scam"]

    #extract and prepare the elements of interest for the investigation
    investigated_element = " ".join(json_dataset_sample["own"]["metadata"]["directory name"].replace("fictional_","").split("_"))
    is_criminal = False
    for crime in list_of_crimes:
        if crime in investigated_element:
            is_criminal = True
    ...
    prompt='\n'.join([introduction_messages_text, json_dataset_sample["combined"]["new_dialogue"].replace("\n\n","\r\n"), request_text])
    if type_sample=="manual":
        answer_summary = json_dataset_sample["own"]["new_summaries"]["new_precise"]
    elif type_sample=="automatic":
        answer_summary = json_dataset_sample["own"]["new_summaries"]["new_GPT4_adjusted"]
    else:
        print("Problem with the type")
...
def main():
    ...
    #setup path for the requests files
    ...
    for dataset_name in datasets:
        ...
        for task in tasks:
            ...
            for creation in creations:
                ...
                #setup the ouput path for each fine-tuning training dataset
                train_output = f"./output_prompt_generation/{dataset_name}/{task}/{creation}/train.json"
                ...
                #setup the ouput path for each fine-tuning training dataset
                train_output_check = f"../../data/tuning/{dataset_name}/{task}/{creation}/train.json"
                
                #setup the ouput path for each fine-tuning testing dataset
                test_output_check = f"../../data/tuning/{dataset_name}/{task}/{creation}/test.json"

                
                #load the training and testing chat summarization datasets
                train_input = f"./input_prompt_generation/{dataset_name}/train.json"
                test_input = f"./input_prompt_generation/{dataset_name}/test.json"
                training_dataset = load_json(train_input)
                testing_dataset = load_json(test_input)

                #setup the lists that will receive the fine-tuning training dataset for each task
                ft_list_training = []
                ...
                #generate the fine-tuning training samples for each task based on the current chat summarization dataset
                for sample in training_dataset:
                    if task =="task-1":
                        ...
                        ft_list_training.append(ft_sample)
                    elif task=="task-2":
                        ...
                        if ft_sample[0]:
                            track_crime+=1
                        ft_list_training.append(ft_sample[1])
                    else:
                ...
                print(len(ft_list_training))
                print(len(ft_list_testing))
                print(f"Number of tracked crimes is {str(track_crime)}")


                #saving the fine-tuning training dataset for each task
                dump_json(ft_list_training,train_output)
                ...
                #saving the fine-tuning training dataset for each task
                dump_json(ft_list_training,train_output_check)
