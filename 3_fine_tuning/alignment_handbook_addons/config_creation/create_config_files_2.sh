...
for model in "${models[@]}"; do
    ...
    repo_model="./recipes/custom/real_full/${model}"
    ...
    for task in "${tasks[@]}"; do
        ...
        for sample in "${samples[@]}"; do
            ...
            if [ "$sample" = "60-samples" ]; then
                dataset=$(echo -e "  GaetanMichelet/chat-60_ft_${task}: 1.0")
            elif [ "$sample" = "120-samples" ]; then
                dataset=$(echo -e "  GaetanMichelet/chat-60_ft_${task}: 1.0\n  GaetanMichelet/chat-120_ft_${task}: 1.0")
            elif [ "$sample" = "180-samples" ]; then
                dataset=$(echo -e "  GaetanMichelet/chat-60_ft_${task}: 1.0\n  GaetanMichelet/chat-120_ft_${task}: 1.0\n  GaetanMichelet/chat-180_ft_${task}: 1.0")
            else
                echo "Problem"
            fi     
            ...
            for config in "${configs[@]}"; do
                ...
                name_combo="${model}_${task}_${sample}_${config}_full"

                repo_file="./recipes/custom/real_full/${model}/${model}_${task}_${sample}_${config}_full"
                mkdir -p "${repo_file}"
