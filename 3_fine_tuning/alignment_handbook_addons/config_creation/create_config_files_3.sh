...
for model in "${models[@]}"; do
    ...
    repo_model="./recipes/custom/real_auto/${model}"
    ...
    for task in "${tasks[@]}"; do
        ...
        for sample in "${samples[@]}"; do
            ...
            if [ "$sample" = "60-samples" ]; then
                dataset=$(echo -e "  GaetanMichelet/chat-60_ft_${task}_auto: 1.0")
            elif [ "$sample" = "120-samples" ]; then
                dataset=$(echo -e "  GaetanMichelet/chat-60_ft_${task}_auto: 1.0\n  GaetanMichelet/chat-120_ft_${task}_auto: 1.0")
            elif [ "$sample" = "180-samples" ]; then
                dataset=$(echo -e "  GaetanMichelet/chat-60_ft_${task}_auto: 1.0\n  GaetanMichelet/chat-120_ft_${task}_auto: 1.0\n  GaetanMichelet/chat-180_ft_${task}_auto: 1.0")
            else
                echo "Problem"
            fi     
            ...
            for config in "${configs[@]}"; do
                ...
                name_combo="${model}_${task}_${sample}_${config}_auto"

                repo_file="./recipes/custom/real_auto/${model}/${model}_${task}_${sample}_${config}_auto"
                mkdir -p "${repo_file}"
                ...
                if [ "$config" = "config-1" ]; then
                    ...
                    learning_rate="1.0e-04"
                    nb_epoch=50
                elif [ "$config" = "config-3" ]; then
                    gradient_accumulation="8"
                    learning_rate="1.0e-05"
                    nb_epoch=150
                elif [ "$config" = "config-4" ]; then
                    gradient_accumulation="16"
                    learning_rate="1.0e-05"
                    nb_epoch=150
                else
